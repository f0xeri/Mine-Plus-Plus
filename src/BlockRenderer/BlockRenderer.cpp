...
typedef union //Тип с фикисрованной точкой
{
    int32_t i;		//Биты числа
    struct
    {
        int16_t lo; // Младшая часть числа, endian-specific!
        int16_t hi;	// Старшая часть числа
    };
} fixed_point;

typedef struct //Тип, определяющий некоторые характеристики отрезка
{
    fixed_point f;
    int32_t m;
    bool isHorisontal;
...
void BlockRenderer::render(ChunkManager &chunks, int cx, int cz, bool frontAngle, std::vector<glm::vec3> &camPoints, Shader &shader, int &renderedChunks)
{
    Chunk *chunk;
    Mesh *mesh;
    glm::mat4 model;
    ...
    if (frontAngle)
    {
        minx = camPoints[2].x / CHUNK_SIZE;
        minz = camPoints[2].z / CHUNK_SIZE;
        maxx = camPoints[3].x / CHUNK_SIZE;
        maxz = camPoints[3].z / CHUNK_SIZE;
    }
    else
    {
        minx = camPoints[0].x / CHUNK_SIZE;
        minz = camPoints[0].z / CHUNK_SIZE;
        maxx = camPoints[1].x / CHUNK_SIZE;
        maxz = camPoints[1].z / CHUNK_SIZE;
    }
    ...
    centroid /= camPoints.size();
    

    chunk = chunks.chunksDict.at({minx, minz});
    mesh = chunk->mesh;
    model = glm::mat4(1.0f);
    model = translate(model, glm::vec3(chunk->x * CHUNK_SIZE, chunk->y * CHUNK_Y, chunk->z * CHUNK_SIZE));
    shader.uniformMatrix(model, "model");
    mesh->draw();
    renderedChunks++;

    chunk = chunks.chunksDict.at({maxx, maxz});
    mesh = chunk->mesh;
    model = glm::mat4(1.0f);
    model = translate(model, glm::vec3(chunk->x * CHUNK_SIZE, chunk->y * CHUNK_Y, chunk->z * CHUNK_SIZE));
    shader.uniformMatrix(model, "model");
    ...
    chunk = chunks.chunksDict.at({cx, cz});
    mesh = chunk->mesh;
    model = glm::mat4(1.0f);
    model = translate(model, glm::vec3(chunk->x * CHUNK_SIZE, chunk->y * CHUNK_Y, chunk->z * CHUNK_SIZE));
    shader.uniformMatrix(model, "model");
    ...
    chunk = chunks.chunksDict.at({centroid.x / CHUNK_SIZE, centroid.z / CHUNK_SIZE});
    mesh = chunk->mesh;
    model = glm::mat4(1.0f);
    model = translate(model, glm::vec3(chunk->x * CHUNK_SIZE, chunk->y * CHUNK_Y, chunk->z * CHUNK_SIZE));
    shader.uniformMatrix(model, "model");
